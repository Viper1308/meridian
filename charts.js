/* ════════════════════════════════════════════════════════════
   CHARTS — hand-rolled SVG. No library, no dependency.
   Every function takes plain data and returns an SVG string.
   ════════════════════════════════════════════════════════════ */
const Chart = (() => {
  let gid = 0;
  const g = () => 'g' + (++gid);
  ...
  function area(series, opts) {
    opts = opts || {};
    const w = 720, h = 260, pl = 42, pr = 12, pt = 14, pb = 28;
    ...
    segs.forEach(s => {
      ...
      if (len > 0.5) {
        body += `<circle cx="${c}" cy="${c}" r="${r}" fill="none" stroke="${s.colour}"
          stroke-width="${sw}" stroke-dasharray="${len - 2} ${circ - len + 2}"
